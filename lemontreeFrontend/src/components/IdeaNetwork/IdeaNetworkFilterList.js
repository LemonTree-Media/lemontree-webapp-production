    ...
    return (
        <div className="mx-5 mt-4 mb-5">
          ...
          <Filter
              ...
              handleChange = {(ops) => {setVideoType(ops);}}
          />
          <Filter
              name="Industry"
              options={OPTIONS["Industry"]}
              handleChange = {(ops) => {setIndustry(ops);}}
          />
          <Filter
              name="Video Style"
              options={OPTIONS["Video Style"]}
              handleChange = {(ops) => {setVideoStyle(ops);}}
          />
          <Filter
              name="Budget"
              options={OPTIONS["Budget"]}
              handleChange = {(ops) => {setBudget(ops);}}
          />

          <Button onClick={queryFilter}>Filter Ideas</Button>
        </div>
    );
}
