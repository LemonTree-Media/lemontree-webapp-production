 ...
 */
ideasRouter.route('/:ideaId/caseStudy') 
.options(cors.corsWithOptions, (req, res) => {res.sendStatus(200);})
.get(cors.cors, (req, res, next) =>{
	Ideas.findById(req.params.ideaId)
	.then(idea => { 
 ...
 */
ideasRouter.route('/:ideaId/caseStudy/:caseStudyId') 
.options(cors.corsWithOptions, (req, res) => {res.sendStatus(200);})
.put(cors.cors, authenticate.verifyUser, (req, res, next) => {
    Ideas.findById(req.params.ideasId)
    .then((ideas) => {
        if (ideas != null && ideas.caseStudy.id(req.params.caseStudyId) != null) {
            if (req.body.description) {
                ideas.caseStudy.id(req.params.caseStudyId).description = req.body.description;                
            }
            if (req.body.brandchallenge) {
                ideas.caseStudy.id(req.params.caseStudyId).brandchallenge = req.body.brandchallenge;                
            }
            if (req.body.views) {
            ...
            if (req.body.daysfinished) {
                ideas.caseStudy.id(req.params.caseStudyId).daysfinished = req.body.daysfinished;                
            }
            if (req.body.assetsdelivered) {
                ideas.caseStudy.id(req.params.caseStudyId).assetsdelivered = req.body.assetsdelivered;                
            }
            if (req.body.channels) {
                ideas.caseStudy.id(req.params.caseStudyId).channels = req.body.channels;                
            }
            ideas.save()
            .then((ideas) => {
            ...
            err = new Error('caseStudy ' + req.params.caseStudyId + ' not found');
            err.status = 404;
            return next(err);            
        }
    }, (err) => next(err))
